...
class BTDriver implements GlobalConst {
	...
	protected void runAllTests() {
		...
		try {
			...
			file = new BTreeFile("AAA" + postfix, keyType, 4, DeleteFashion.FULL_DELETE);// full delete
		} catch (Exception e) {
			e.printStackTrace();
			return;
		}
			...
			try {
				...
				case 3:
					...
					if (flag == 0) {
						...
						System.out.println("Initially 100 values are inserted from 1 to 100.");
	...
	void test5(int n, int m) throws Exception {
		try {
			...
			System.out.println(" ***************** The file name is: " + "AAA" + postfix + "  **********");
			file = new BTreeFile("AAA" + postfix, keyType, 20, DeleteFashion.NAIVE_DELETE);
			file.traceFilename("TRACE");
